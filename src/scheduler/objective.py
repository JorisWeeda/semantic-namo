...
class Objective:
    ...
    def _rotation_to_goal(self, sim, indices, n=1):
        rotation_goal_indices = torch.clamp(indices + n, max=len(self.waypoints) - 1)
        ...
        tar_yaw = torch.atan2(delta_y, delta_x)  # Shape: (m,)
        rob_yaw = sim.dof_state[:, n]

        yaw_diff = torch.abs(tar_yaw - rob_yaw)  # Shape: (m,)

        rotation_norm_cost_per_env = yaw_diff / torch.pi  # Shape: (m,)
        return rotation_norm_cost_per_env  # Shape: (m,)
    ...
    def _contact_forces_to_goal(self, epsilon=1e-3):
        interaction_norm_per_env = self._cumulative_contact_force / (torch.max(self._cumulative_contact_force) + epsilon)
